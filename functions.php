...
/* Fire our meta box setup function on the post editor screen. */
add_action( 'load-post.php', 'directory_slider_setup' );
add_action( 'load-post-new.php', 'directory_slider_setup' );

/* Meta box setup function. */
function directory_slider_setup() {

  /* Add meta boxes on the 'add_meta_boxes' hook. */
  add_action( 'add_meta_boxes', 'directory_slider_meta_boxes' );

  /* Save post meta on the 'save_post' hook. */
  add_action( 'save_post', 'directory_slider_save_meta', 10, 2 );
}

function directory_slider_meta_boxes() {

  add_meta_box(
    'directory_slider_meta_box_class',      // Unique ID
    esc_html__( 'Directory image and video slider', 'example' ),    // Title
    'directory_slider_meta_box',   // Callback function
    'directory',         // Admin page (or post type)
    'normal',         // Context
    'high'         // Priority
  );
...
/* Display the post meta box. */
function directory_slider_meta_box( $object, $box ) { ?>
  <?php wp_nonce_field( basename( __FILE__ ), 'directory_slider_meta_box_class_nonce' ); ?>
  <p>
    <label for="directory_slider_meta_box_class"><?php _e( "Add a slider shortcode from royal slider for the listing", 'example' ); ?></label>
    <br />
	<textarea style="height: 40px;" class="widefat" id="directory_slider_meta_box_class" name="directory_slider_meta_box_class"><?php echo get_post_meta( $object->ID, 'directory_slider_meta_box_class', true ); ?></textarea>
  </p>
<?php }

/* Save the meta box's post metadata. */
function directory_slider_save_meta( $post_id, $post ) {

  /* Verify the nonce before proceeding. */
  if ( !isset( $_POST['directory_slider_meta_box_class_nonce'] ) || !wp_verify_nonce( $_POST['directory_slider_meta_box_class_nonce'], basename( __FILE__ ) ) )
    return $post_id;
  ...
  /* Get the posted data and sanitize it for use as an HTML class. */
  $new_meta_value = ( isset( $_POST['directory_slider_meta_box_class'] ) ? ( $_POST['directory_slider_meta_box_class'] ) : '' );

  /* Get the meta key. */
  $meta_key = 'directory_slider_meta_box_class';
